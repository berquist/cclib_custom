...
from parse_matrices_dalton import parse_matrix_dalton, parse_element_dalton
    ...
    def extract(self, inputfile, line):
        ...
        if "Integrals of operator:" in line:
            ...
            if not hasattr(self, "AOPROPER"):
                self.AOPROPER = dict()
            if not any(ignore in line for ignore in self.ignore_these_matrices):
                ...
                # representation!
                is_antisymmetric = any(
                    re.search(ail, matname) for ail in self.antisymmetric_integral_labels
                )
                mat_sparse = parse_matrix_dalton(inputfile)
                mat = sparse_to_dense_matrix_dalton(
                    mat_sparse, self.nbasis, self.nbasis, antisymmetric=is_antisymmetric
                )
        ...
        if line.strip() == "MO. COEFFICIENTS FOR SYMMETRY    1":
            ...
            mocoeffs_list = []
            mocoeffs_full = sparse_to_dense_matrix_dalton(
                mocoeffs_sparse, self.nbasis, self.nbasis, mocoeffs=True
            )
...
def orbdia(UDV, FOCK, FC, FV, norb, nclosed, nact, nvirt, AVDIA=False):
    ...
    for m in r_active:
        for i in r_inactive:
            eodia[m, i] += (
                2 * FC[m, m] + UDV[m, m] * FC[i, i] - FOCK[i, i] - FOCK[m, m] + 2 * FV[m, m]
            )
            if AVDIA:
